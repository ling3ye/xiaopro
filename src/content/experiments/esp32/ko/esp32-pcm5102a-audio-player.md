...
**ESP32-S3 + PCM5102A** 조합은 DIY 오디오 프로젝트 중 가성비가 뛰어난 선택 중 하나입니다. ESP32-S3이 Wi-Fi 연결, MP3 가져오기, 오디오 스트림 디코딩을 담당하고, PCM5102A가 디지털 신호를 아날로그 오디오로 변환합니다（참고: 순수 DAC 출력으로 앰프가 내장되어 있지 않아 스피커 연결 시 외부 앰프 모듈이 필요합니다）. 전체 비용이 몇 천 원에 불과하지만, 동일 가격대 제품을 훨씬 능가하는 음질을 제공합니다.
...
ESP32-S3에 전원을 연결하면 자동으로 Wi-Fi에 연결되고, 네트워크에서 MP3 오디오 스트림을 가져와 PCM5102A로 디코딩하여 출력합니다. 이어폰이나 외부 앰프+스피커에서 소리가 재생됩니다. 터치스크린이나 버튼 조작 없이 전원만 연결하면 바로 재생이 시작됩니다.
...
ESP32-S3이 출력하는 것은 **디지털 오디오 신호**（I2S 형식의 0과 1）이지만, 앰프, 이어폰 등 오디오 기기는 디지털 신호를 이해하지 못합니다. 이해할 수 있는 것은 **아날로그 전압 신호**（시간에 따라 변화하는 파형）뿐입니다. PCM5102A는 이 둘 사이의 "통역사" 역할을 하여, 디지털 신호를 실시간으로 오디오 기기가 이해할 수 있는 아날로그 신호로 변환합니다.
...
| 출력 | 차동 라인 레벨 출력（앰프 없음, 스피커 직접 구동 불가） |
...
> **⚠️ 중요：PCM5102A는 순수 DAC 모듈로 앰프가 내장되어 있지 않습니다！**
>
> PCM5102A는 디지털 신호를 아날로그 신호（Line Level）로 변환만 합니다. **증폭 기능이 전혀 없습니다.**
>
> - **스피커를 직접 연결하지 마세요**：스피커 임피던스가 낮아（보통 4Ω～8Ω） PCM5102A의 출력 전류로는 구동할 수 없으며, 직접 연결하면**모듈이 타버립니다**.
> - **이어폰 연결도 위험합니다**：대부분의 이어폰 임피던스가 낮아（16Ω～32Ω） PCM5102A의 라인 레벨 출력은 이어폰 구동용으로 설계되지 않았으며, 장시간 사용하면 모듈이 손상될 수 있습니다. 고임피던스 이어폰（≥250Ω）은 위험이 낮지만 여전히 권장하지 않습니다.
> - **올바른 방법**：스피커를 연결하려면 PCM5102A와 스피커 사이에 **앰프 모듈**（PAM8403, MAX98357, TPA2016 등）을 추가하세요. 테스트 목적이라면 고임피던스 이어폰을 사용하고 볼륨을 낮게 유지하세요.
...
| 고임피던스 이어폰 | × 1 | 테스트용, ≥64Ω 권장; 저임피던스 이어폰이나 스피커는 외부 앰프 모듈（PAM8403 등） 필요 |
...
void setup() {
  ...
  audio.setVolume(12);  // 볼륨 범위 0～21, 10 정도에서 시작하여 서서히 올리세요. 이어폰이나 모듈 손상 방지
...
- `audio.setVolume(12)`：볼륨 범위는 0～21입니다. 10 정도에서 시작하여 서서히 올리세요. 볼륨이 너무 높으면 PCM5102A의 출력 전류가 증가하여 이어폰 사용 시 모듈 손상 위험이 있습니다.
...
이것은 ESP32 + PCM5102A 프로젝트에서 가장 흔한 고장으로, 주요 원인은 **I2S DMA 버퍼가 너무 작아서** 디코딩이나 데이터 읽기 속도가 I2S 출력 속도를 따라가지 못해 버퍼 언더런이 발생하기 때문입니다.

#### 신구 버전 설정 방법 비교（매우 중요！）

**현재 주류 버전（V3.x / master 브랜치）：**
- 더 이상 `i2s_config_t`를 직접 사용하지 않습니다.
- 매크로 정의（`#define`）또는 `audio.settings` 구조체를 사용합니다.
- master 버전을 다운로드한 일부 사용자는 매크로 충돌로 인해 컴파일 에러가 발생할 수 있습니다（`'class Audio' has no member named 'settings'`）.

#### 권장 해결 방법（현재 라이브러리 버전과 호환, 바로 컴파일 가능）

`setup()` 맨 앞, **`setPinout()` 이전**에 다음 코드를 추가하세요：
...
void setup() {
    ...
    // ==================== 【핵심】I2S DMA 버퍼 증가 ====================
    #undef DMA_DESC_NUM     // 라이브러리 기본 매크로 정의 해제
    #undef DMA_FRAME_NUM
    #define DMA_DESC_NUM  16   // 버퍼 수（권장 12~24）
    #define DMA_FRAME_NUM 512  // 버퍼당 프레임 수（권장 256~1024）

    audio.setPinout(I2S_BCLK, I2S_LRCK, I2S_DOUT);

    // 기타 설정
    audio.setVolume(21);
    // audio.connecttohost(...) 또는 audio.connecttoFS(...)
}
```

**권장 매개변수（PCM5102A 실제 경험）：**

| 시나리오           | DMA_DESC_NUM | DMA_FRAME_NUM | 비고                             |
|--------------------|--------------|---------------|----------------------------------|
| 시작 추천          | 16           | 512           | 대부분의 경우 눈에 띄게 개선     |
| 더 안정적（추천）  | 20           | 768           | PSRAM 있을 때 더 효과적          |
| 최고 안정성        | 24           | 1024          | 네트워크 불안정 또는 고비트레이트 시 |
| 메모리 부족        | 12           | 384           | PSRAM 없는 일반 ESP32          |

**구버전（V2.x 이전）：**
- 전통적인 `i2s_config_t` 구조체를 사용합니다.
- 매개변수명은 `dma_buf_count`와 `dma_buf_len`입니다.
- 일반적으로 라이브러리 소스 코드 수정 또는 초기화 시 구조체 재정의가 필요합니다.
...
### Q：PCM5102A의 출력에 스피커나 이어폰을 직접 연결할 수 있나요？

**스피커는 직접 연결하지 마세요. 이어폰도 주의가 필요합니다.** PCM5102A는 순수 DAC 모듈로 라인 레벨（Line Level） 출력만 제공하며 증폭 기능이 없습니다. 스피커（4Ω～8Ω）를 직접 연결하면 과전류로 **모듈이 타버립니다**. 저임피던스 이어폰（16Ω～32Ω）도 장시간 사용하면 손상 위험이 있습니다.

스피커를 연결하려면 PCM5102A 출력단에 앰프 모듈을 추가해야 합니다. 추천：PAM8403（3W×2, 저렴하고好用）、MAX98357（I2S 직접 입력, 내장 DAC로 PCM5102A 대체 가능）、TPA2016（2W×2, AGC 내장）. 테스트 시에는 고임피던스 이어폰（≥64Ω）을 사용하고 볼륨을 낮게 설정하세요.
