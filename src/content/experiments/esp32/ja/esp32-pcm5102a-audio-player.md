...
これは ESP32 + PCM5102A プロジェクトで最もよくある故障であり、主な原因は **I2S DMA バッファが小さすぎる** ことで、デコードやデータ読み取り速度が I2S 出力速度に追いつかず、バッファアンダーランが発生します。

#### 新旧バージョンの設定方法の比較（重要！）

**現在の主流バージョン（V3.x / master ブランチ）：**
- `i2s_config_t` を直接使用しなくなりました。
- マクロ定義（`#define`）または `audio.settings` 構造体を使用します。
- master バージョンをダウンロードした一部のユーザーは、マクロの競合によりコンパイルエラーが発生する可能性があります（`'class Audio' has no member named 'settings'`）。

#### 推奨される解決方法（現在のライブラリバージョンと互換、直接コンパイル可能）

`setup()` の先頭、**`setPinout()` の前** に以下のコードを追加してください：
...
void setup() {
    ...
    // ==================== 【重要】I2S DMA バッファを増やす ====================
    #undef DMA_DESC_NUM     // ライブラリのデフォルトマクロ定義を解除
    #undef DMA_FRAME_NUM
    #define DMA_DESC_NUM  16   // バッファ数（推奨 12~24）
    #define DMA_FRAME_NUM 512  // バッファあたりのフレーム数（推奨 256~1024）

    audio.setPinout(I2S_BCLK, I2S_LRCK, I2S_DOUT);

    // その他の設定
    audio.setVolume(21);
    // audio.connecttohost(...) または audio.connecttoFS(...)
}
```

**推奨パラメータ（PCM5102A 実践経験）：**

| シナリオ           | DMA_DESC_NUM | DMA_FRAME_NUM | 備考                             |
|--------------------|--------------|---------------|----------------------------------|
| 最初の推奨         | 16           | 512           | ほとんどの場合で明らかな改善     |
| より安定（推奨）   | 20           | 768           | PSRAM 搭載時はさらに効果的       |
| 最高の安定性       | 24           | 1024          | ネットワーク不安定または高ビットレート時 |
| メモリが限られる   | 12           | 384           | PSRAM なしの通常 ESP32          |

**旧バージョン（V2.x 以前）：**
- 従来の `i2s_config_t` 構造体を使用。
- パラメータ名は `dma_buf_count` と `dma_buf_len`。
- 通常、ライブラリのソースコードの修正または初期化時の構造体の再定義が必要。
