...
**ESP32-S3 + PCM5102A** 是目前 DIY 音訊專案裡性價比最高的組合之一：ESP32-S3 負責連 Wi-Fi、拉取 MP3、解碼音訊串流，PCM5102A 負責把數位訊號轉換成類比音訊輸出（注意：這是純 DAC 輸出，無功放，接喇叭需外加功放模組）。整個方案成本不超過幾十塊，音質卻遠超同價位方案。
...
ESP32-S3 上電後自動連接 Wi-Fi，從網路拉取 MP3 音訊串流，透過 PCM5102A 解碼輸出（透過耳機或外接功放+喇叭播放出聲音）。全程無需觸控螢幕、無需按鍵，上電即播。
...
你的 ESP32-S3 輸出的是 **數位音訊訊號**（I2S 格式的 0 和 1），但功放、耳機等音訊裝置聽不懂數位訊號，它們只認 **類比電壓訊號**（隨時間變化的波形）。PCM5102A 就是這兩者之間的「同步口譯」，把數位訊號即時翻譯成音訊裝置能理解的類比訊號。
...
| MCLK | 內建 PLL，無需外部主時脈 |
| 輸出 | 差分線路級輸出（Line Level，無功放，不能直驅喇叭） |

**為什麼選 PCM5102A？** 便宜、好用、3.3V 直接驅動、不需要外部時脈、動態範圍 112dB 在單晶片音訊裡相當能打——是 ESP32 專案裡最常見的 I2S DAC 搭檔。

> **⚠️ 重要：PCM5102A 是純 DAC 模組，沒有功放！**
>
> PCM5102A 只負責把數位訊號轉換成類比訊號（Line Level 線路級電平），**它本身沒有任何放大能力**。
>
> - **絕對不能直接接喇叭（揚聲器）**：喇叭阻抗低（通常 4Ω～8Ω），PCM5102A 的輸出電流遠不足以驅動，直接接會**燒毀模組**。
> - **接耳機也有風險**：大部分耳機的阻抗較低（16Ω～32Ω），PCM5102A 的線路級輸出並非設計用來驅動耳機，長時間使用可能損壞模組。高阻抗耳機（≥250Ω）風險相對較低，但仍不推薦。
> - **正確做法**：如果需要接喇叭，必須在 PCM5102A 和喇叭之間加一級**功放模組**（如 PAM8403、MAX98357、TPA2016 等）。如果只是調試測試，建議用高阻抗耳機且音量不要開太大。
...
| 高阻抗耳機 | × 1 | 調試用，建議 ≥64Ω；低阻抗耳機或喇叭需外加功放模組（如 PAM8403） |
...
void setup() {
  ...
  audio.setVolume(12);  // 音量範圍 0～21，建議從 10 開始逐步調高，避免損傷耳機或模組
...
- `audio.setVolume(12)`：音量範圍 0～21，建議從 10 左右開始逐步調高。音量過大會增加 PCM5102A 的輸出電流，接耳機時尤其注意不要開太大。
...
這是 ESP32 + PCM5102A 專案中最常見的故障，主要原因是 **I2S DMA 緩衝區太小**，導致解碼或資料讀取速度跟不上 I2S 輸出速度，產生 Buffer Underrun（資料欠載）。

#### 新舊版本設定方式對比（非常重要！）

**目前主流版本（V3.x / master 分支）：**
- 不再直接使用 `i2s_config_t`。
- 採用巨集定義（`#define`）或 `audio.settings` 結構體。
- 部分使用者下載的 master 版本中，`audio.settings` 可能因巨集衝突導致編譯錯誤（`'class Audio' has no member named 'settings'`）。

#### 推薦解決方法（相容你目前的函式庫版本，可直接編譯通過）

在 `setup()` 最前面、**`setPinout()` 之前** 加入以下程式碼：
...
void setup() {
    ...
    // ==================== 【關鍵】增大 I2S DMA 緩衝區 ====================
    #undef DMA_DESC_NUM     // 先取消函式庫預設的巨集定義
    #undef DMA_FRAME_NUM
    #define DMA_DESC_NUM  16   // 緩衝區數量（推薦 12~24）
    #define DMA_FRAME_NUM 512  // 每緩衝區幀數（推薦 256~1024）
...
| 場景               | DMA_DESC_NUM | DMA_FRAME_NUM | 說明                     |
|--------------------|--------------|---------------|--------------------------|
| 起步推薦           | 16           | 512           | 大多數情況已明顯改善     |
| 更穩定（推薦）     | 20           | 768           | 有 PSRAM 時效果更好      |
| 最高穩定           | 24           | 1024          | 網路不穩或高位元率時使用 |
| 記憶體緊張         | 12           | 384           | 普通 ESP32 無 PSRAM 時   |

**舊版本（V2.x 及更早）：**
- 使用傳統的 `i2s_config_t` 結構體。
- 參數名為 `dma_buf_count` 和 `dma_buf_len`。
- 通常需要修改函式庫原始碼或在初始化時重新定義結構體。
...
### Q：PCM5102A 的輸出可以直接接喇叭或耳機嗎？

**不能直接接喇叭，接耳機也需要謹慎。** PCM5102A 是純 DAC 模組，輸出的是線路級電平（Line Level），沒有放大能力。直接接喇叭（4Ω～8Ω）會因為電流過大而**燒毀模組**。接低阻抗耳機（16Ω～32Ω）長時間使用也有損壞風險。

如果需要接喇叭，必須在 PCM5102A 輸出端加一級功放模組，推薦搭配：PAM8403（3W×2，便宜好用）、MAX98357（I2S 直入，自帶 DAC 可替換 PCM5102A）、TPA2016（2W×2，帶 AGC）。調試階段建議使用高阻抗耳機（≥64Ω）且音量調低。
