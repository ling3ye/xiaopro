...
> **本文核心：** 使用 ESP32-S3 开发板，通过 I2S 接口连接 PCM5102A DAC 模块，配合 ESP32-audioI2S 库，实现 Wi-Fi 在线 MP3 播放。接线不超过 10 根，代码不超过 50 行，小白可直接上手。
...
**ESP32-S3 + PCM5102A** 是目前 DIY 音频项目里性价比最高的组合之一：ESP32-S3 负责连 Wi-Fi、拉取 MP3、解码音频流，PCM5102A 负责把数字信号转换成模拟音频输出到耳机或音箱。整个方案成本不超过几十块，音质却远超同价位方案。
...
ESP32-S3 上电后自动连接 Wi-Fi，从网络拉取 MP3 音频流，通过 PCM5102A 解码输出，耳机或音箱里播放出声音。全程无需触屏、无需按键，上电即播。
...
你的 ESP32-S3 输出的是 **数字音频信号**（I2S 格式的 0 和 1），但耳机和音箱听不懂数字信号，它们只认 **模拟电压信号**（随时间变化的波形）。PCM5102A 就是这两者之间的"同声传译"，把数字信号实时翻译成音频设备能理解的模拟信号。
...
<!-- 建议在此插入接线示意图 -->
<!-- alt="ESP32-S3 连接 PCM5102A I2S 接线图" -->
...
void setup() {
  ...
  audio.setVolume(18);  // 音量范围 0～21，18 是个舒服的值
...
- `audio.setVolume(18)`：音量范围 0～21，18 是默认测试值，可自行调整。
...
### Q：PCM5102A 的输出接口是什么？可以接功放吗？

PCM5102A 模块通常提供标准 3.5mm 立体声模拟音频输出，可直接接耳机或无源音箱。如果需要接功放，建议使用模块上的 LINE OUT 接口，输出电平更适合功放输入，音质也更好。
