...
> 难度：⭐⭐☆☆☆（有一点基础就能上手） 预计时间：30 分钟 测试环境：Arduino IDE 2.3.8 + ESP32 Arduino Core 3.3.10
...
如果还没装板子包，在 Arduino IDE → 工具 → 开发板管理器 里搜 `esp32`，安装 **Espressif Systems** 出的那个包（本文测试版本：**ESP32 Arduino Core 3.3.10**）。
